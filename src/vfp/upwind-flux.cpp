...
sapphirepp::VFP::UpwindFlux<dim, has_momentum, logarithmic_p>::UpwindFlux(
  ...
  , double_dummy{1.}
{
  ref_units_ = solver_control.reference_units;

  const double m0  = ref_units_.mass;
  const double c0  = ref_units_.velocity;
  const double q0  = ref_units_.charge;
  const double t0  = ref_units_.time;                 // = 1/omega_g
  const double pi  = dealii::numbers::PI;
  const double mu0 = 4.0 * pi * 1.0e-7;              

  const double tau_s_si =
      (9.0 * pi * std::pow(m0, 3) * c0) / (std::pow(q0, 4) * mu0);

  // Dimensionless coefficient: (3/2)*(t0/tau_s)*(q^4/m^3)
  coeff = 1.5 * (t0 / tau_s_si) * std::pow(charge, 4) / std::pow(mass, 3);
  ...
  prepare_work_arrays_for_lapack()
{
  ...
  dealii::Vector<double>   magnetic_field_dummy{1., 1., 1.};
  for (unsigned int i = 0; i < jacobian_dummy.m(); ++i)
    for (unsigned int j = 0; j < jacobian_dummy.n(); ++j)
      jacobian_dummy[i][j] = 1.;

  compute_matrix_sum(
		     n_p_dummy, p_dummy, gamma_dummy, material_derivative_dummy, jacobian_dummy, magnetic_field_dummy);
  // call Lapack routine
  dealii::syevr(jobz,
                range,
                uplo,
      ...
      if constexpr ((vfp_flags & VFPFlags::synchrotron) != VFPFlags::none)
        {
          ...
          const double Bx = B_vec[0];
          const double By = B_vec[1];
          const double Bz = B_vec[2];
          
          const double B2 = Bx*Bx + By*By + Bz*Bz;
              
          // matrix_sum is flattened row-major; accumulate S_syn into it
          unsigned k = 0;
          for (unsigned r = 0; r < matrix_size; ++r)
            for (unsigned c = 0; c < matrix_size; ++c, ++k)
              {
              double S_rc = 0.0;
              S_rc += Bx*Bx * adv_mat_products[0][k];
              S_rc += 2.0*Bx*By * adv_mat_products[1][k];
              S_rc += 2.0*Bx*Bz * adv_mat_products[2][k];
              S_rc += By*By * adv_mat_products[3][k];
              S_rc += 2.0*By*Bz * adv_mat_products[4][k];
              S_rc += Bz*Bz * adv_mat_products[5][k];
              if (r == c) S_rc -= B2;           
              matrix_sum[k] += n_p * coeff * gamma * S_rc;   
              }
        }
    }
  else
    ...
    if constexpr ((vfp_flags & VFPFlags::synchrotron) != VFPFlags::none)
        {
          ...
          for (unsigned r = 0; r < matrix_size; ++r)
            for (unsigned c = 0; c < matrix_size; ++c, ++k)
              ...
              S_rc += Bz*Bz * adv_mat_products[5][k];
              if (r == c) S_rc -= B2;           
              matrix_sum[k] += n_p * coeff * gamma * momentum * S_rc;    
              }
        }
}
                              ...
                              dealii::FullMatrix<double> &negative_flux_matrix)
{
  ...
  // variable matrix_sum
  compute_matrix_sum(n_p, momentum, gamma, material_derivative, jacobian, B_vec);
