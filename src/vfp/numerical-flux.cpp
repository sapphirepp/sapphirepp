  ...
  if (logarithmic_p)
    {
      ...
      for (unsigned k = 0; k < matrix_size * matrix_size; ++k)
        {
          matrix_sum[k] =
            -n_p *
            ((gamma / p) * (material_derivative[0] * advection_matrices[0][k] +
                            material_derivative[1] * advection_matrices[1][k] +
                            material_derivative[2] * advection_matrices[2][k]) +
             jacobian[0][0] * adv_mat_products[0][k] +
             jacobian[1][1] * adv_mat_products[3][k] +
             jacobian[2][2] * adv_mat_products[5][k] +
             (jacobian[0][1] + jacobian[1][0]) * adv_mat_products[1][k] +
             (jacobian[0][2] + jacobian[2][0]) * adv_mat_products[2][k] +
             (jacobian[1][2] + jacobian[2][1]) * adv_mat_products[4][k]);

          if constexpr ((vfp_flags & VFPFlags::synchrotron) != VFPFlags::none)
            {
              double S = (B_vec[0] * B_vec[0]) * adv_mat_products[0][k] +
                         2.0 * (B_vec[0] * B_vec[1]) * adv_mat_products[1][k] +
                         2.0 * (B_vec[0] * B_vec[2]) * adv_mat_products[2][k] +
                         (B_vec[1] * B_vec[1]) * adv_mat_products[3][k] +
                         2.0 * (B_vec[1] * B_vec[2]) * adv_mat_products[4][k] +
                         (B_vec[2] * B_vec[2]) * adv_mat_products[5][k];

              if (k % (matrix_size + 1) == 0)
                S -= (B_vec[0] * B_vec[0] + B_vec[1] * B_vec[1] +
                      B_vec[2] * B_vec[2]);

              matrix_sum[k] += n_p * coeff * gamma * S;
            }
        }
    }
  else
      ...
      for (unsigned k = 0; k < matrix_size * matrix_size; ++k)
        {
          matrix_sum[k] =
            -n_p *
            (gamma * mass *
               (material_derivative[0] * advection_matrices[0][k] +
                material_derivative[1] * advection_matrices[1][k] +
                material_derivative[2] * advection_matrices[2][k]) +
             momentum *
               (jacobian[0][0] * adv_mat_products[0][k] +
          ...
          if constexpr ((vfp_flags & VFPFlags::synchrotron) != VFPFlags::none)
            {
              ...
              matrix_sum[k] += n_p * coeff * gamma * momentum * S;
            }
        }
    }
}
                              ...
                              dealii::FullMatrix<double> &negative_flux_matrix)
{
  ...
  // variable matrix_sum
  compute_matrix_sum(
    n_p, momentum, gamma, material_derivative, jacobian, B_vec);
