...
After having set `DEAL_II_DIR`, sapphire can be compiled with the following
commands
...
On the MPIK cluster dealii with its dependencies is installed for all members of
APT under

```
/lfs/l8/tap/tap/tapshared/lib/dealii
```
...
and set following set of variables needs to be set
...
After having set the `DEAL_II_DIR`, loaded the modules and set the above variables,
call the above cmake command to compile sapphire.
...
store the output of simulation runs. The physical input allows a user to set
...
- The `Mesh` (or triangulation) section offers the following options:
    ...
    - `Number of cells` determines in how many cells the domain will divided in each
...
- The `Time Stepping` section offers the following options:
    ...
    - `Time step size` determines the time step size. This option is crucial for the
      stability of the simulation run. If too large, the simulation will become
      unstable.
...
- The `Output` section offers
    ...
    - `Simulation identifier` is the name of the current simulation run. A folder
      with this name will created in the results folder and the output of this run
...
user only sets the reference energy, the other two quantities are computed. The
reference values are set in `include/reference-values.h`. There default values
are
...
The initial condition, the fields, the source and the scattering frequency are
must be provided as analytical expressions (and not, for example, as data in a
file). All of them are defined in the file `src/physical-setup.cpp`. The
following examples demonstrate how to implement the physical setup in sapphire.
...
velocity field, which depends on \(x\). This expressed with using the first
coordinate in `point` , i.e. `point[0]`. When sapphire needs to know
the velocity \(\mathbf{u}\), it calls `vector_value` and it hands over the
...
```
For time dependent magnetic fields it works exactly the same. Note that
...
the coefficients with \(l > 0\) encode the anisotropies of the distribution.
The initial value function allows to set all coefficients, but most of the time
it is enough to set first one. The following example demonstrates how to this. 
...
last component of `point` is corresponds to the \(p\)-direction For example, if
`terms` include the `momentum` flag and if `dim_configuration_space` is set to
one, then `point[2]` is \(p\). If `dim_configuration_space` was two, then
`point[3]` would be \(p\). 
...
case of the initial conditions, the last component of `point` represents the
momentum, if momentum terms are included.
...
```
The signature of this function is now different, because the scattering
frequency is a scalar. Hence, it is not a vector which is returned, but a value.
Moreover, it is implemented such that it takes multiple points at once (i.e. it
takes a vector of points) and it returns the corresponding values as vector as
well.
...
void Sapphire::ScatteringFrequency<dim>::value_list(
  ...
  for(unsigned int i = 0; i < points.size(); ++i) {
	scattering_frequencies[i] = 0.3/points[i][dim -1]
  }
}
```
gives the relation \(\nu = 0.3/p\).
