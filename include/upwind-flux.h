...
class UpwindFlux {
 ...
 private:
  ...
  // The support of the eigenvectors in "eigenvectors" array, i.e., the indices
  // indicating the nonzero elements in "eigenvectors" array
  ...
  //
  // An approximate eigenvalue is accepted as converged when it is determined to
  // lie in an interval [a,b] of width less than or equal to
  //                 ABSTOL + EPS *   max( |a|,|b| ) ,
  ...
  const double abstol;
  // If range = 'A', no upper, nor lower bounds for the eigenvalues are needed.
  // It is still necessary to hand over dummies, which are not referenced when
