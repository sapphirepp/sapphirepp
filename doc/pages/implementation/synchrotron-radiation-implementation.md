# Synchrotron cooling {#synchrotron-radiation-impl}
...
\f[
\begin{split}
&+ \int_{\partial T} \boldsymbol{\Phi}_{i} \cdot
\left[\!
...
\begin{split}
&+ \int_{\partial T} \boldsymbol{\Phi}_{i}\cdot
\frac{3 q_*^4 \gamma}{2\bar{\tau}_{s*} m_*^2}
\left(\mathbf{A}^{a}\mathbf{A}^{b}B_{a*}B_{b*}-B_*^2\mathbf{1}\right)
\zeta_{j}\boldsymbol{\Phi}_{j} n_{\ln p*} \\
&{}- \int_{T} \frac{\partial \boldsymbol{\Phi}_{i}}{\partial \ln p_*}\cdot
\frac{3 q_*^4 \gamma}{2\bar{\tau}_{s*} m_*^2}
\left(\mathbf{A}^{a}\mathbf{A}^{b}B_{a*}B_{b*}-B_*^2\mathbf{1}\right)
\zeta_{j}\boldsymbol{\Phi}_{j} \\
&{}+ \int_{T} \boldsymbol{\Phi}_{i}\cdot
...
| Previous                          | Next               |
| :-------------------------------- | :----------------- |
| [Boundary conditions](#boundary-conditions) | [Implementation](#implementation) |
