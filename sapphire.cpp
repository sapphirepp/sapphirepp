...
namespace Sapphire
{
  ...
  private:
    ...
    // like to maintain the possibility to compute 1D scenarios. In Step-17,
    // Step-18 it is explained how to deal with copys of the triangulation and
    // the dof handler on every mpi process: It is enough to replace
    // parallel::distributed:Triangulation with parallel::shared::Triangulation.
    // We use a bit of C++ metaprogramming tricky to decide which triangulation
    // to use.
  ...
  VFPEquationSolver::run()
  {
    ...
    // Here a non ghosted vector, is copied into a ghosted vector. I think that
    // is the moment where the ghost cells are filled.
  ...
  void
  VFPEquationSolver::setup_system()
  {
    ...
    // Vectors
    locally_owned_previous_solution.reinit(locally_owned_dofs,
                                           mpi_communicator);
  ...
  void
  VFPEquationSolver::assemble_mass_matrix()
  {
  ...
  void
  VFPEquationSolver::assemble_dg_matrix(const double time)
  {
              ...
              for (unsigned int j : fe_v.dof_indices())
                {
                  const unsigned int component_j =
                    fe_v.get_fe().system_to_component_index(j).first;
                  if constexpr ((flags & TermFlags::collision) !=
                                TermFlags::none)
                    {
                      if (component_i == component_j)
                        {
                          ...
                          copy_data.cell_matrix(i, j) +=
                            frequencies[q_index] *
                            collision_matrix[component_i] *
                            fe_v.shape_value(i, q_index) *
                            fe_v.shape_value(j, q_index) * JxW[q_index];
                        }
                    }
                  // NOTE: If spatial advection term is deactivated, then dim_cs
                  // must euqal zero, i.e. the distributin function is assumend
                  // to be homogeneous (it does not depend on x, y and z). And
                  // if dim_cs = 0, there for loop is not entered.
                      ...
                      if (component_i == component_j)
                        {
                          ...
                          // - Ay_ij) ] \phi_i \phi_j where \partial_x/y Ax/y_ij
                          // = 0
                          copy_data.cell_matrix(i, j) -=
                  ...
                  if constexpr ((flags & TermFlags::magnetic) !=
                                TermFlags::none)
                    {
                      // NOTE: All three components of the B-Field are included
                      // no matter, which dimension of the configuration space
                      // is considered
                  ...
                  if constexpr ((flags & TermFlags::momentum) !=
                                TermFlags::none)
                    {
              ...
              for (unsigned int j = 0; j < n_facet_dofs; ++j)
                {
                  ...
                  if constexpr ((flags & TermFlags::spatial_advection) !=
                                TermFlags::none)
                    {
                      // Outflow boundary: Everyhing with a positive flux along
                      // the direction of the normal leaves the boundary
                      copy_data.cell_matrix(i, j) +=
  ...
  template <int dim>
  void
  VFPEquationSolver::project(const Function<dim>        &f,
                             PETScWrappers::MPI::Vector &projected_function)
  {
  ...
  template <int dim>
  void
  VFPEquationSolver::compute_source_term(const Function<dim> &source_function)
  {
    FEValues<dim_ps> fe_v(mapping,
                          fe,
                          quadrature,
        ...
        if (cell->is_locally_owned())
          {
            ...
            constraints.distribute_local_to_global(
              cell_rhs, local_dof_indices, locally_owned_current_source);
          }
      }
  ...
  void
  VFPEquationSolver::theta_method(const double time, const double time_step)
  {
    ...
    if constexpr ((flags & TermFlags::source) != TermFlags::none)
      {
        if constexpr (time_dependent_source)
          {
            system_rhs.add((1 - theta) * time_step,
  ...
  void
  VFPEquationSolver::explicit_runge_kutta(const double time,
                                          const double time_step)
  {
    ...
    // ERK4 method. I cannot use locally_owned previous solution, because it is
  ...
  void
  VFPEquationSolver::low_storage_explicit_runge_kutta(const double time,
                                                      const double time_step)
  {
    ...
    // cannot be written to. It is necessary to use a vector that does not
