...
namespace Sapphire
{
  namespace VFP
  {
    ...
    VFPEquationSolver::assemble_mass_matrix()
    {
      ...
      ScratchData<dim_ps> scratch_data(mapping,
                                       fe,
                                       quadrature,
                                       quadrature_face);
      CopyData            copy_data;
      // Perfom the integration loop only over the locally owned cells
      const auto filtered_iterator_range = dof_handler.active_cell_iterators() |
    ...
    VFPEquationSolver::assemble_dg_matrix(const double time)
    {
      ...
      */
      ...
      const Vector<double> &collision_matrix =
        pde_system.get_collision_matrix();
                        ...
                        if constexpr (logarithmic_p)
                          {
                            ...
                            for (unsigned int coordinate_1 = 0;
                                 coordinate_1 < 3;
                                 ++coordinate_1)
                              {
                                ...
                                for (unsigned int coordinate_2 = 0;
                                     ...
                                     ++coordinate_2)
                                  {
                                    ...
                                    copy_data.cell_matrix(i, j) +=
                                      ...
                                      jacobians_vel[q_index][coordinate_1]
                                                   [coordinate_2] *
                                      t_matrices[coordinate_2 * 3 +
                                                 coordinate_1](component_i,
                                                               component_j) *
                                      fe_v.shape_value(j, q_index) *
                                      JxW[q_index];
                                  }
        ...
        for (unsigned int q_index : fe_v_face.quadrature_point_indices())
          {
            ...
            for (unsigned int i : fe_v_face_neighbor.dof_indices())
              {
                const unsigned int component_i = fe_v_face_neighbor.get_fe()
